...
const Dashboard = () => {
  ...
  <div className='shadow py-4'>
    <div className='px-5 flex justify-between items-center'>
      ...
      <div className='flex items-center gap-3'>
        <p className='max-sm:hidden'>welcome, NextEra Coders</p>
        <div className='relative group'> {/* corrected to 'group' */}
          <img className='w-8 border rounded-full' src={assets.company_icon} alt="" /> {/* corrected border class */}
  ...
  <div className='flex items-start'>
    {/* Left sidebar with option to add job, manage jobs, view applictions */}
    <div className='inline-block min-h-screen border-r-2'>
      <ul className='flex flex-col items-start pt-5 text-gray-800'>
        <NavLink className={({isActive})=> ` flex items-center p-3 sm:px-6 gap-2 w-full hover:bg-gray-100 ${isActive && 'bg-blue-100 border-r-4 border-blue-500'}`} to={'/dashboard/add-job'}>
            <img className='minl-w-4' src={assets.add_icon} alt="" />
        ...
        <NavLink className={({isActive})=> ` flex items-center p-3 sm:px-6 gap-2 w-full hover:bg-gray-100 ${isActive && 'bg-blue-100 border-r-4 border-blue-500'}`} to={'/dashboard/manage-job'}>
            <img className='minl-w-4' src={assets.home_icon} alt="" />
        ...
        <NavLink className={({isActive})=> ` flex items-center p-3 sm:px-6 gap-2 w-full hover:bg-gray-100 ${isActive && 'bg-blue-100 border-r-4 border-blue-500'}`} to={'/dashboard/view-applications'}>
            <img className='minl-w-4' src={assets.person_tick_icon} alt="" />
